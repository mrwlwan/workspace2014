...
class Register_corp:
    def __init__(self):
        ...
        self.charset = 'gbk'
        self.opener = Opener(charset=self.charset)
        # 6位机关代码; 6位
        self.org_code = '440103'
        ...
        self.reg = re.compile(r'称：</td><td[^>]*>(?P<name>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<addr>[^<]*)</td>[^：]*：</td><td[^>]*><font color="red">(?P<register_code>[^<]*)</font></td>[^：]*：</td><td[^>]*>(?P<representative>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<nature>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<period>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<capital>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<register_department>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<status>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<establishment_data>[^<]*)</td>[^：]*：</td><td[^>]*>(?P<scope>[^<]*)</td>', re.S)
        self._save_times = 0
    ...
    def _process(self, corp_dict):
        corp_dict['insert_date'] = datetime.date.today()
    ...
    def fetch(self, code):
        ...
        content = self.opener.open(url, timeout=10, raw_string=False)
        #if content.find(self.search_text) < 0:
        if self.search_text_reg.match(content):
            print('没找到相关信息.')
            return []
        result = []
        for search_obj in self.reg.finditer(content):
    ...
    def action_new(self, invalid_times=200):
        for query_obj in self.max_code_model.get_all():
    ...
    def report(self):
        ...
        #corps = self.model.filter_by(status='登记成立', insert_date=datetime.date(2011,11,1))
        rows = []
        fields = (
            ('名称', 'name'),
            ('注册码', 'register_code'),
