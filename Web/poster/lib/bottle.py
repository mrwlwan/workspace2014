...
class BaseResponse(object):
    ...
    def iter_headers(self):
        ...
        headers = iter(list(self._headers.items()))
...
class MultiDict(DictMixin):
    ...
    def iterkeys(self): return iter(list(self.dict.keys()))
    def itervalues(self): return (v[-1] for v in list(self.dict.values()))
    def iteritems(self): return ((k, v[-1]) for (k, v) in list(self.dict.items()))
...
class SimpleTemplate(BaseTemplate):
    ...
    def prepare(self, escape_func=html.escape, noescape=False):
        self.cache = {}
        enc = self.encoding
    ...
    def code(self):
        ...
        template = self.source if self.source else open(self.filename, encoding='utf8').read()
