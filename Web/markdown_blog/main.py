...
from models import Entry, Tag, EntryTag, STATUS, KINDS
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def render_string(self, template_name, **kwargs):
        return super(BaseHandler, self).render_string(template_name, users=users, KINDS=KINDS, STATUS=STATUS, Entry=Entry, Tag=Tag, **kwargs)
...
class Homehandler(BaseHandler):
    ...
    def get(self, count=4):
        ...
        blogs=KINDS.get('blog').entry_set.filter('status =', STATUS['active']).order('-published').fetch(count)
        newses=KINDS.get('news').entry_set.filter('status =', STATUS['active']).order('-published').fetch(count)
        remarks=KINDS.get('remark').entry_set.filter('status =', STATUS['active']).order('-published').fetch(count)
...
class EntryListHandler(BaseHandler):
    ...
    def get(self, kind_name):
        ...
        items = kind.entry_set.filter('status =', STATUS['active']).order('-published')
...
class EntryHandler(BaseHandler):
    ...
    def get(self, slug):
        ...
        if entry.status.name=='active' or self.current_user and self.current_user.is_admin:
            self.set_json(entry)
            entry.pv_increase()
...
class ComposeHandler(BaseHandler):
    ...
    def _add_tags(self, entry, tags):
        reg = re.compile(r'\s*[,，][\s,，]*')
        tag_names = set((tag_name.capitalize() for tag_name in reg.split(re.sub(r'\s+', ' ', tags.lower())) if tag_name))
        ...
        for tag_name in tag_names:
            ...
            logging.info(tag)
            if not tag:
                logging.info('create new tag %s', tag_name)
                tag = Tag(name=tag_name)
                tag.put()
            tag_dict[tag_name] = tag
        for entry_tag in entry.entrytag_set:
            name =  entry_tag.tag.name
            if name in tag_dict:
                tag_dict.pop(name)
            else:
                entry_tag.remove_entry_tag()
        for tag in tag_dict.values():
            EntryTag.create_entry_tag(tag=tag, entry=entry)
...
class FeedHandler(BaseHandler):
    def get(self, limit=10):
        entries = db.Query(Entry).filter('status =', STATUS['active']).order('-published').fetch(limit)
...
class AdminEntryHandler(BaseHandler):
    ...
    def get(self):
        ...
        status = self.get_argument('status', None)
        if status in STATUS:
            entries=STATUS.get(status).entry_set.filter('entry_kind =', KINDS.get('blog')).order('-published')
            self.render('admin_list_show.html', status=status, entries=entries, total_pages=entries.count())
    ...
    def post(self):
        ...
        if kind_name not in KINDS:
            return
        kind = KINDS.get(kind_name)
        entries = kind.entry_set.filter('status =', status).order('-published')
        self.render('modules/admin_entry_list.html', entries=entries)
...
application = tornado.wsgi.WSGIApplication([
    ...
    (r'/admin/entry', AdminEntryHandler),
], **settings)
