...
class Config:
    def __init__(self, config_file=None):
        self.config_file = config_file or 'config.cfg'
        if not os.path.exists(self.config_file):
            open(config_file, 'w', encoding='utf8').close()
        self.parser = configparser.ConfigParser(allow_no_value=True)
        self.parser.read(self.config_file, encoding='utf8')
    ...
    def get_section_config(self, section_name):
        return dict(self.get_parser()[section_name])
